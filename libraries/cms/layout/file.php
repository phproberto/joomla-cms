...
class JLayoutFile extends JLayoutBase
{
	...
	protected function getPath()
	{
		...
		if (is_null($this->fullPath) && !empty($this->layoutId))
		{
			...
			{
				if ($this->layoutId == 'test.layout')
				{
					echo '<pre>empty: '; print_r($this->includePaths); echo '</pre>';
				}

				$this->loadDefaultIncludePaths();
			}
			elseif ($this->layoutId == 'test.layout')
			{
				echo '<pre>exist: '; print_r($this->includePaths); echo '</pre>';
			}
	 ...
	 *
	 * @return  JLayoutFile  Self instance for chaing
	 *
	 * @since   3.5
	 */
